...
enc_loop_generic_64_inner (const uint8_t **s, uint8_t **o, const uint16_t *enc_table)
{
	uint64_t src;
	...
	memcpy(*o + 0, enc_table + index0, 2);
	memcpy(*o + 2, enc_table + index1, 2);
	memcpy(*o + 4, enc_table + index2, 2);
	memcpy(*o + 6, enc_table + index3, 2);
...
static inline void
enc_loop_generic_64 (const uint8_t **s, size_t *slen, uint8_t **o, size_t *olen, const uint16_t *enc_table)
{
	if (*slen < 8) {
		return;
	}
	...
	*olen += rounds * 8;	// 8 bytes produced per round
	...
	do {
		if (rounds >= 8) {
			enc_loop_generic_64_inner(s, o, enc_table);
			enc_loop_generic_64_inner(s, o, enc_table);
			enc_loop_generic_64_inner(s, o, enc_table);
			enc_loop_generic_64_inner(s, o, enc_table);
			enc_loop_generic_64_inner(s, o, enc_table);
