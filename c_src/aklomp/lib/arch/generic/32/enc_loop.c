...
enc_loop_generic_32_inner (const uint8_t **s, uint8_t **o, const uint16_t *enc_table)
{
	uint32_t src;
...
static inline void
enc_loop_generic_32 (const uint8_t **s, size_t *slen, uint8_t **o, size_t *olen, const uint16_t *enc_table)
{
	if (*slen < 4) {
		return;
	}
	...
	*olen += rounds * 4;	// 4 bytes produced per round
	...
	do {
		if (rounds >= 8) {
			enc_loop_generic_32_inner(s, o, enc_table);
			enc_loop_generic_32_inner(s, o, enc_table);
			enc_loop_generic_32_inner(s, o, enc_table);
			enc_loop_generic_32_inner(s, o, enc_table);
			enc_loop_generic_32_inner(s, o, enc_table);
